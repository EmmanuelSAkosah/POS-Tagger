...
def build_corpus(corpus):
    # tuples is a list of string
    tuples = list(itertools.chain.from_iterable(corpus)) #unpacked chained elements in corpus,
    words = [tuple[0].lower for tuple in tuples] #get all words
    poses = [tuple[1] for tuple in tuples] # all possible tags
    word_counter = collections.Counter(words)
    vocab = [tuple[0] for tuple in word_counter.most_common(n=(VOCAB_SIZE -2))] #get most common words
    pos_counter = collections.Counter(poses)
    tags = [tuple[0] for tuple in pos_counter.most_common(n=(TAGS_SIZE-2))]
    word2id = dict(zip(vocab, range(1,len(vocab)+1))) #associate words with ids
    word2id['PAD'] = 0 #mark paddings and unkowns
    ...
    for i, record in enumerate(corpus):
        ...
        #weed out words and tags not in most common vocab
    ...
    print("store dictionary to disk:")
